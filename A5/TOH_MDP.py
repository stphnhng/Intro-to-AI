...
This version includes hooks to run a script and for doing comparisons
of the results of Q-Learning with results of Value Iteration.

'''

import hungs3_VI as VI
import hungs3_Q_Learn as Q_Learn
...
def train_until(criterion="Policy match on golden path", threshold=1, max_iterations=10000):
...
def set_up_state_space():
  ...
  Vis.basic_plot(STATES_AND_EDGES, N_disks, MDP_command)
...
# The following must come after the call to set_up_state_space,
# so it can add to an existing GUI.
if Compare_QLearn_to_VI:
  Vis.add_compare_menu_item()
